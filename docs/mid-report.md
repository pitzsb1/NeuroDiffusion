...
본 프로젝트는 EEG 시계열 데이터를 기반으로 diffusion 모델을 학습하여, 실제 EEG 신호의 분포를 모사하고 희귀/이상 EEG 데이터를 생성(증강)하는 것을 목표로 함. 특히 EEG 데이터의 클래스 불균형 문제(이상 신호 부족)를 해결하기 위해 생성 모델 기반 데이터 증강 접근을 수행함.
...
단순 Conv 구조보다 장기적인 시간 패턴 학습이 가능한 U-Net이 시계열의 **multi-scale 구조 학습에 유리**함
